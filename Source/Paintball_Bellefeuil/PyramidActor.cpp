...
APyramidActor::APyramidActor()
{
	...
	pyramidMesh = CreateDefaultSubobject  <UProceduralMeshComponent>(TEXT("GeneratedMesh"));
...
void APyramidActor::PostActorCreated() {
	Super::PostActorCreated();
	GenerateBoxMesh();
}

// This is called when actor is already in level and map is opened. 
void APyramidActor::PostLoad() {
	Super::PostLoad();
	GenerateBoxMesh();

}

void APyramidActor::GenerateBoxMesh() {
	...
	pyramidMesh->CreateMeshSection(0, Vertices, Triangles, Normals, TextureCoordinates, Colors, Tangents, true);
}
void APyramidActor::CreateBoxMesh(FVector BoxRadius, TArray < FVector > & Vertices, TArray < int32 > & Triangles, TArray < FVector > & Normals, TArray < FVector2D > & UVs, TArray < FProcMeshTangent > & Tangents, TArray < FColor > & Colors) {
	FVector BoxVerts[8];
	//Specifies vertices of cube
	//Top half of cube
	BoxVerts[0] = FVector(-BoxRadius.X / 3, BoxRadius.Y / 3, BoxRadius.Z / 3); //Front right
	BoxVerts[1] = FVector(BoxRadius.X / 3, BoxRadius.Y / 3, BoxRadius.Z / 3); //Back right
	BoxVerts[2] = FVector(BoxRadius.X / 3, -BoxRadius.Y / 3, BoxRadius.Z / 3); //Back left
	BoxVerts[3] = FVector(-BoxRadius.X / 3, -BoxRadius.Y / 3, BoxRadius.Z / 3);  //Front left
	//Bottom half of cube
	BoxVerts[4] = FVector(-BoxRadius.X * 3, BoxRadius.Y * 3, -BoxRadius.Z * 3); //Front right
	BoxVerts[5] = FVector(BoxRadius.X * 3, BoxRadius.Y * 3, -BoxRadius.Z * 3); //Back right
	BoxVerts[6] = FVector(BoxRadius.X * 3, -BoxRadius.Y * 3, -BoxRadius.Z * 3); //Back left
	BoxVerts[7] = FVector(-BoxRadius.X * 3, -BoxRadius.Y * 3, -BoxRadius.Z * 3); //Front left
