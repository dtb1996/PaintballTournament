...
#include "WallRunActor.h"


// Sets default values
AWallRunActor::AWallRunActor()
{
 	// Set this actor to call Tick() every frame.  You can turn this off to improve performance if you don't need it.
	//PrimaryActorTick.bCanEverTick = true;

	wallMesh = CreateDefaultSubobject  <URuntimeMeshComponent>(TEXT("GeneratedMesh"));
	RootComponent = wallMesh;

}

// Called when the game starts or when spawned
void AWallRunActor::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void AWallRunActor::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void AWallRunActor::PostActorCreated() {
	Super::PostActorCreated();
	GenerateBoxMesh();
}

// This is called when actor is already in level and map is opened. 
void AWallRunActor::PostLoad() {
	Super::PostLoad();
	GenerateBoxMesh();

}

void AWallRunActor::GenerateBoxMesh() {
	...
	wallMesh->CreateMeshSection(0, Vertices, Triangles, Normals, TextureCoordinates, Colors, Tangents, true);
}
void AWallRunActor::CreateBoxMesh(FVector BoxRadius, TArray < FVector > & Vertices, TArray < int32 > & Triangles, TArray < FVector > & Normals, TArray < FVector2D > & UVs, TArray < FRuntimeMeshTangent > & Tangents, TArray < FColor > & Colors) {
	FVector BoxVerts[8];
	//Specifies vertices of cube
	//Top half of cube
	BoxVerts[0] = FVector(-BoxRadius.X * 100, BoxRadius.Y * 0.5, BoxRadius.Z * 10); //Front right
	BoxVerts[1] = FVector(BoxRadius.X * 100, BoxRadius.Y * 0.5, BoxRadius.Z * 10); //Back right
	BoxVerts[2] = FVector(BoxRadius.X * 100, -BoxRadius.Y * 0.5, BoxRadius.Z * 10); //Back left
	BoxVerts[3] = FVector(-BoxRadius.X * 100, -BoxRadius.Y * 0.5, BoxRadius.Z * 10);  //Front left
	//Bottom half of cube
	BoxVerts[4] = FVector(-BoxRadius.X * 100, BoxRadius.Y * 0.5, -BoxRadius.Z * 10); //Front right
	BoxVerts[5] = FVector(BoxRadius.X * 100, BoxRadius.Y * 0.5, -BoxRadius.Z * 10); //Back right
	BoxVerts[6] = FVector(BoxRadius.X * 100, -BoxRadius.Y * 0.5, -BoxRadius.Z * 10); //Back left
	BoxVerts[7] = FVector(-BoxRadius.X * 100, -BoxRadius.Y * 0.5, -BoxRadius.Z * 10); //Front left 
