...
ABarricadeActor::ABarricadeActor()
{
	...
	barricadeMesh = CreateDefaultSubobject  <UProceduralMeshComponent>(TEXT("GeneratedMesh"));
	RootComponent = barricadeMesh;
...
void ABarricadeActor::GenerateBoxMesh() {
	...
	barricadeMesh->CreateMeshSection(0, Vertices, Triangles, Normals, TextureCoordinates, Colors, Tangents, true);
}
void ABarricadeActor::CreateBoxMesh(FVector BoxRadius, TArray < FVector > & Vertices, TArray < int32 > & Triangles, TArray < FVector > & Normals, TArray < FVector2D > & UVs, TArray < FProcMeshTangent > & Tangents, TArray < FColor > & Colors) {
	FVector BoxVerts[8];
	//Specifies vertices of cube
	//Top half of cube
	BoxVerts[0] = FVector(-BoxRadius.X, BoxRadius.Y * 3, BoxRadius.Z * 3); //Front right
	BoxVerts[1] = FVector(BoxRadius.X, BoxRadius.Y * 3, BoxRadius.Z * 3); //Back right
	BoxVerts[2] = FVector(BoxRadius.X, -BoxRadius.Y * 3, BoxRadius.Z * 3); //Back left
	BoxVerts[3] = FVector(-BoxRadius.X, -BoxRadius.Y * 3, BoxRadius.Z * 3);  //Front left
	//Bottom half of cube
	BoxVerts[4] = FVector(-BoxRadius.X, BoxRadius.Y * 3, -BoxRadius.Z * 3); //Front right
	BoxVerts[5] = FVector(BoxRadius.X, BoxRadius.Y * 3, -BoxRadius.Z * 3); //Back right
	BoxVerts[6] = FVector(BoxRadius.X, -BoxRadius.Y * 3, -BoxRadius.Z * 3); //Back left
	BoxVerts[7] = FVector(-BoxRadius.X, -BoxRadius.Y * 3, -BoxRadius.Z * 3); //Front left
