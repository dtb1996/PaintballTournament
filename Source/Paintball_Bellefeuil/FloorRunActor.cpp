...
#include "FloorRunActor.h"


// Sets default values
AFloorRunActor::AFloorRunActor()
{
 	// Set this actor to call Tick() every frame.  You can turn this off to improve performance if you don't need it.
	//PrimaryActorTick.bCanEverTick = true;

	floorMesh = CreateDefaultSubobject  <URuntimeMeshComponent>(TEXT("GeneratedMesh"));
	RootComponent = floorMesh;

}

// Called when the game starts or when spawned
void AFloorRunActor::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void AFloorRunActor::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void AFloorRunActor::PostActorCreated() {
	Super::PostActorCreated();
	GenerateBoxMesh();
}

// This is called when actor is already in level and map is opened. 
void AFloorRunActor::PostLoad() {
	Super::PostLoad();
	GenerateBoxMesh();

}

void AFloorRunActor::GenerateBoxMesh() {
	...
	floorMesh->CreateMeshSection(0, Vertices, Triangles, Normals, TextureCoordinates, Colors, Tangents, true);
}
void AFloorRunActor::CreateBoxMesh(FVector BoxRadius, TArray < FVector > & Vertices, TArray < int32 > & Triangles, TArray < FVector > & Normals, TArray < FVector2D > & UVs, TArray < FRuntimeMeshTangent > & Tangents, TArray < FColor > & Colors) {
	FVector BoxVerts[8];
	//Specifies vertices of cube
	//Top half of cube
	BoxVerts[0] = FVector(-BoxRadius.X * 100, BoxRadius.Y * 80, BoxRadius.Z * 0.5); //Front right
	BoxVerts[1] = FVector(BoxRadius.X * 100, BoxRadius.Y * 80, BoxRadius.Z * 0.5); //Back right
	BoxVerts[2] = FVector(BoxRadius.X * 100, -BoxRadius.Y * 80, BoxRadius.Z * 0.5); //Back left
	BoxVerts[3] = FVector(-BoxRadius.X * 100, -BoxRadius.Y * 80, BoxRadius.Z * 0.5);  //Front left
	//Bottom half of cube
	BoxVerts[4] = FVector(-BoxRadius.X * 100, BoxRadius.Y * 80, -BoxRadius.Z * 0.5); //Front right
	BoxVerts[5] = FVector(BoxRadius.X * 100, BoxRadius.Y * 80, -BoxRadius.Z * 0.5); //Back right
	BoxVerts[6] = FVector(BoxRadius.X * 100, -BoxRadius.Y * 80, -BoxRadius.Z * 0.5); //Back left
	BoxVerts[7] = FVector(-BoxRadius.X * 100, -BoxRadius.Y * 80, -BoxRadius.Z * 0.5); //Front left 
