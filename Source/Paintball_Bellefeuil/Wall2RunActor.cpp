//Dillon Bellefeuille
//GAM-415
//This class builds 3D objects in the game world with dimensions specified in code
//The number of actors, location, and materials are picked in the editor
//This satisfies requirement GAM-415-01: Display 3D objects in game development environments by employing graphics application program interfaces

#include "Wall2RunActor.h"


// Sets default values
AWall2RunActor::AWall2RunActor()
{
 	// Set this actor to call Tick() every frame.  You can turn this off to improve performance if you don't need it.
	//PrimaryActorTick.bCanEverTick = true;

	wall2Mesh = CreateDefaultSubobject  <URuntimeMeshComponent>(TEXT("GeneratedMesh"));
	RootComponent = wall2Mesh;

}

// Called when the game starts or when spawned
void AWall2RunActor::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void AWall2RunActor::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void AWall2RunActor::PostActorCreated() {
	Super::PostActorCreated();
	GenerateBoxMesh();
}

// This is called when actor is already in level and map is opened. 
void AWall2RunActor::PostLoad() {
	Super::PostLoad();
	GenerateBoxMesh();

}

void AWall2RunActor::GenerateBoxMesh() {
	...
	wall2Mesh->CreateMeshSection(0, Vertices, Triangles, Normals, TextureCoordinates, Colors, Tangents, true);
}
void AWall2RunActor::CreateBoxMesh(FVector BoxRadius, TArray < FVector > & Vertices, TArray < int32 > & Triangles, TArray < FVector > & Normals, TArray < FVector2D > & UVs, TArray < FRuntimeMeshTangent > & Tangents, TArray < FColor > & Colors) {
	FVector BoxVerts[8];
	//Specifies vertices of cube
	//Top half of cube
	BoxVerts[0] = FVector(-BoxRadius.X * 0.5, BoxRadius.Y * 79, BoxRadius.Z * 10); //Front right
	BoxVerts[1] = FVector(BoxRadius.X * 0.5, BoxRadius.Y * 79, BoxRadius.Z * 10); //Back right
	BoxVerts[2] = FVector(BoxRadius.X * 0.5, -BoxRadius.Y * 79, BoxRadius.Z * 10); //Back left
	BoxVerts[3] = FVector(-BoxRadius.X * 0.5, -BoxRadius.Y * 79, BoxRadius.Z * 10);  //Front left
	//Bottom half of cube
	BoxVerts[4] = FVector(-BoxRadius.X * 0.5, BoxRadius.Y * 79, -BoxRadius.Z * 10); //Front right
	BoxVerts[5] = FVector(BoxRadius.X * 0.5, BoxRadius.Y * 79, -BoxRadius.Z * 10); //Back right
	BoxVerts[6] = FVector(BoxRadius.X * 0.5, -BoxRadius.Y * 79, -BoxRadius.Z * 10); //Back left
	BoxVerts[7] = FVector(-BoxRadius.X * 0.5, -BoxRadius.Y * 79, -BoxRadius.Z * 10); //Front left 
