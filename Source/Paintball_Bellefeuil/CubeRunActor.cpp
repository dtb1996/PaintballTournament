...
#include "CubeRunActor.h"


// Sets default values
ACubeRunActor::ACubeRunActor()
{
...
// Called every frame
void ACubeRunActor::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void ACubeRunActor::PostActorCreated() {
	Super::PostActorCreated();
	GenerateBoxMesh();
}

// This is called when actor is already in level and map is opened. 
void ACubeRunActor::PostLoad() {
	Super::PostLoad();
	GenerateBoxMesh();

}

void ACubeRunActor::GenerateBoxMesh() {
...
void ACubeRunActor::CreateBoxMesh(FVector BoxRadius, TArray < FVector > & Vertices, TArray < int32 > & Triangles, TArray < FVector > & Normals, TArray < FVector2D > & UVs, TArray < FRuntimeMeshTangent > & Tangents, TArray < FColor > & Colors) {
	FVector BoxVerts[8];
	//Top half of cube
	BoxVerts[0] = FVector(-BoxRadius.X, BoxRadius.Y, BoxRadius.Z); //Front right
	BoxVerts[1] = FVector(BoxRadius.X, BoxRadius.Y, BoxRadius.Z); //Back right
	BoxVerts[2] = FVector(BoxRadius.X, -BoxRadius.Y, BoxRadius.Z); //Back left
	BoxVerts[3] = FVector(-BoxRadius.X, -BoxRadius.Y, BoxRadius.Z);  //Front left
	//Bottom half of cube
	BoxVerts[4] = FVector(-BoxRadius.X, BoxRadius.Y, -BoxRadius.Z); //Front right
	BoxVerts[5] = FVector(BoxRadius.X, BoxRadius.Y, -BoxRadius.Z); //Back right
	BoxVerts[6] = FVector(BoxRadius.X, -BoxRadius.Y, -BoxRadius.Z); //Back left
	BoxVerts[7] = FVector(-BoxRadius.X, -BoxRadius.Y, -BoxRadius.Z); //Front left 
