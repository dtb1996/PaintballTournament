...
#include "Barricade2Actor.h"


// Sets default values
ABarricade2Actor::ABarricade2Actor()
{
 	// Set this actor to call Tick() every frame.  You can turn this off to improve performance if you don't need it.
	//PrimaryActorTick.bCanEverTick = true;

	barricade2Mesh = CreateDefaultSubobject  <UProceduralMeshComponent>(TEXT("GeneratedMesh"));
	RootComponent = barricade2Mesh;

}

// Called when the game starts or when spawned
void ABarricade2Actor::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void ABarricade2Actor::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void ABarricade2Actor::PostActorCreated() {
	Super::PostActorCreated();
	GenerateBoxMesh();
}

// This is called when actor is already in level and map is opened. 
void ABarricade2Actor::PostLoad() {
	Super::PostLoad();
	GenerateBoxMesh();

}

void ABarricade2Actor::GenerateBoxMesh() {
	...
	barricade2Mesh->CreateMeshSection(0, Vertices, Triangles, Normals, TextureCoordinates, Colors, Tangents, true);
}
void ABarricade2Actor::CreateBoxMesh(FVector BoxRadius, TArray < FVector > & Vertices, TArray < int32 > & Triangles, TArray < FVector > & Normals, TArray < FVector2D > & UVs, TArray < FProcMeshTangent > & Tangents, TArray < FColor > & Colors) {
	FVector BoxVerts[8];
	//Specifies vertices of cube
	//Top half of cube
	BoxVerts[0] = FVector(-BoxRadius.X, BoxRadius.Y * 2, BoxRadius.Z); //Front right
	BoxVerts[1] = FVector(BoxRadius.X, BoxRadius.Y * 2, BoxRadius.Z); //Back right
	BoxVerts[2] = FVector(BoxRadius.X, -BoxRadius.Y * 2, BoxRadius.Z); //Back left
	BoxVerts[3] = FVector(-BoxRadius.X, -BoxRadius.Y * 2, BoxRadius.Z);  //Front left
	//Bottom half of cube
	BoxVerts[4] = FVector(-BoxRadius.X, BoxRadius.Y * 2, -BoxRadius.Z); //Front right
	BoxVerts[5] = FVector(BoxRadius.X, BoxRadius.Y * 2, -BoxRadius.Z); //Back right
	BoxVerts[6] = FVector(BoxRadius.X, -BoxRadius.Y * 2, -BoxRadius.Z); //Back left
	BoxVerts[7] = FVector(-BoxRadius.X, -BoxRadius.Y * 2, -BoxRadius.Z); //Front left
